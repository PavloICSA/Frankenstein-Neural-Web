...
static void init_network(int n_inputs, int n_hidden, int n_outputs) {
...
static void compute_forward_pass(float* input) {
    ...
    for (int h = 0; h < network.n_hidden; h++) {
        ...
        float z_h = dot_product(input, weights_slice, network.n_inputs);
        z_h += network.bias_h[h];
        
        // Apply sigmoid activation
        network.hidden_activations[h] = sigmoid(z_h);
    }
...
static void compute_backward_pass(float* input, float target, float learning_rate) {
    ...
    for (int h = 0; h < network.n_hidden; h++) {
        ...
        delta_h[h] = error_h * sigmoid_derivative(network.hidden_activations[h]);
    }
...
// Exported training function
EMSCRIPTEN_KEEPALIVE
float train_ann(float* inputs, float* outputs, int n_rows, int n_inputs) {
    // Initialize network with fixed hidden layer size
    int n_hidden = 6;
    int n_outputs = 1;
    init_network(n_inputs, n_hidden, n_outputs);
